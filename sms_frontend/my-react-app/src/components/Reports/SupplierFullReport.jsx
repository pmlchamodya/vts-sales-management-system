...
const SupplierReport = () => {
  ...
  const handleOpenFarmerReport = async () => {
    ...
    try {
      ...
      alert("දත්ත ලබාගැනීම අසාර්ථක විය.");
    }
  };
  ...
  const handleUnprintedBillClick = async (code, bill) => {
    ...
    try {
      ...
      const loanRes = await api.get(
        `/supplier-loan/search?code=${code}&bill_no=${bill || ""}`,
      );
      if (loanRes.data) {
      ...
      if (supRes.data) {
        setAdvanceAmount(supRes.data.advance_amount);
    ...
    return (
      <div style={modernModalOverlay}>
        <div style={modernSelectionCard}>
          ...
          <h2 style={{ color: "#1a2a6c", margin: "20px 0 10px 0" }}>
            සැපයුම්කරු වාර්තාව
          </h2>
          <p
            style={{ color: "#777", fontSize: "0.9rem", marginBottom: "25px" }}
          >
            විස්තරාත්මක වාර්තාව බැලීම සඳහා ගොවි කේතය තෝරන්න
          </p>
          ...
          <div style={{ display: "flex", gap: "15px", marginTop: "30px" }}>
            <button onClick={handleOpenFarmerReport} style={modernSubmitBtn}>
              වාර්තාව බලන්න
            </button>
            <button
              onClick={() => setIsFarmerModalOpen(false)}
              style={modernCancelBtn}
            >
              අවලංගු කරන්න
            </button>
          </div>
        ...
        <div style={dashboardContainer}>
          ...
          <div style={statsGrid}>
            ...
            <div style={{ ...statCard, borderLeft: "6px solid #f6c23e" }}>
              ...
              <h2 style={{ margin: 0, color: "#f6c23e" }}>
                රු. {formatDecimal(profile.advance_amount)}
              </h2>
            </div>
          </div>
                ...
                <table style={modernTable}>
                  ...
                  <tbody>
                    {loans.map((l) => (
                      <tr key={l.id}>
                        <td>
                          <span style={billBadge}>{l.bill_no || "N/A"}</span>
                        </td>
                        <td style={{ color: "#1cc88a", fontWeight: "bold" }}>
  ...
  const filteredPrintedItems = summary.printed.filter((item) =>
    item.supplier_code.toLowerCase().includes(printedSearchTerm.toLowerCase()),
  );
    ...
    <>
      ...
      <nav style={navBarStyle}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
          ...
          <button style={navBtnGreen} onClick={goToSalesEntry}>
            මුල් පිටුව
          </button>
        </div>
      </nav>
              ...
              <div style={listContainerStyle}>
                {filteredPrintedItems.map((item, i) => (
                  <button
                    ...
                    style={printedBtn}
                  >
                    {item.supplier_code}-{item.supplier_bill_no}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={mainDetailsPanel}>
              <div style={headerStyle}>
                <h2 style={{ color: "white" }}>
                  බිල්පත් විස්තර: {selectedBillNo || "..."}
                </h2>
                {profilePic && (
                  <img
                    src={`https://goviraju.lk/vts_sales_backend/application/public/storage/${profilePic}`}
              ...
              {selectedLoanRecord && (
                <div style={loanSummaryBox}>
                  <div>
                    ...
                    <h3 style={{ margin: 0, color: "#1cc88a" }}>
                  ...
                  <div>
                    ...
                    <h3 style={{ margin: 0, color: "#e74a3b" }}>
              ...
              <div style={listContainerStyle}>
                {loanSummary.map((loan, index) => (
                  <button
                    ...
                    style={unprintedBtn}
                  >
                    <strong>{loan.supplier_code}</strong>
                    <br />
                    <small>බිල්: {loan.supplier_bill_no}</small>
                  </button>
                ))}
              </div>
            </div>
