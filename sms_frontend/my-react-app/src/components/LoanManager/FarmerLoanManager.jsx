...
const FarmerLoanManager = () => {
  ...
  // States for the Farmer Breakdown Modal
  ...
  const fetchFarmerBalance = useCallback(async (code) => {
    if (code) {
      try {
        ...
        setTotalLoanDisplay(
          `ශේෂය (Old - Today): ${data.balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
        );
      } catch (error) {
  ...
  const handleViewBreakdown = async () => {
    ...
    try {
      const res = await api.get(
        `/farmer-loans/${form.supplier_code}/breakdown`,
      );
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
            ...
            <div className="row gy-3">
              ...
              <div className="col-md-4">
                ...
                <div className="d-flex gap-2 align-items-start">
                  <div style={{ flexGrow: 1 }}>
                    <Select
                      ...
                      onChange={(opt) =>
                        setForm({
                          ...form,
                          supplier_code: opt ? opt.value : null,
                        })
                      }
                      value={farmers.find(
                        (f) => f.value === form.supplier_code,
                      )}
                      placeholder="සොයන්න..."
                      className="text-dark"
                    />
                  ...
                  <button
                    type="button"
                    className="btn btn-sm btn-light fw-bold text-dark border-dark mt-1"
                ...
                {form.supplier_code && totalLoanDisplay && (
                  <div className="balance-display mt-2">{totalLoanDisplay}</div>
                )}
              </div>
      ...
      {/* RENDER THE FARMER MODAL COMPONENT */}
