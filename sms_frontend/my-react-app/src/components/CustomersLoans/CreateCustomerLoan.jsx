...
const CreateCustomerLoan = ({
  ...
  useEffect(() => {
    if (editingLoan) {
      ...
      setFormData({
        ...
        cheque_date:
          editingLoan.cheque_date || new Date().toISOString().split("T")[0],
      ...
      if (
        editingLoan.customer_id &&
        (editingLoan.loan_type === "today" || editingLoan.loan_type === "old")
      ) {
  ...
  const handleChange = (e) => {
    ...
    if (
      name === "customer_id" &&
      value &&
      (formData.loan_type === "today" || formData.loan_type === "old")
    ) {
  ...
  const updateDescription = () => {
    const { loan_type, bank, wasted_code, wasted_packs, wasted_weight } =
      formData;
    let description = "";
  ...
  const fetchCustomerLoanTotal = async (customerId) => {
    try {
      const response =
        await customerLoanService.getCustomerLoanTotal(customerId);
      const total = Math.abs(response.data.total_amount);
      setTotalAmount(
        `(Total Loans: ${total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`,
      );
    } catch (error) {
      ...
      if (error.response && error.response.status === 422) {
        ...
        setErrors({
          general:
            ...
            `Error ${isEditing ? "updating" : "creating"} loan record`,
        });
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div
      className="border border-2 border-dark rounded p-3 mb-4"
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <div className="row gy-2">
            ...
            formData.loan_type !== "returns" && (
              <div className="row gx-2">
                ...
                <div className="col-md-5">
                  ...
                  />
                  {totalAmount && (
                    <div className="text-white-50 small">{totalAmount}</div>
                  )}
                </div>
              </div>
            )}
          ...
          {/* Cheque Fields */}
          {formData.settling_way === "cheque" &&
            formData.loan_type === "old" && (
