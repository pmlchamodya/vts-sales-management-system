...
const CustomersLoanList = () => {
  ...
  const handleEdit = (loan) => {
    ...
    // Scroll to the form
    document
      .getElementById("loan-form-section")
      ...
      >
        <div className="card-body">
          ...
          <div className="mt-4">
            ...
            <div className="table-responsive">
              <table className="table table-bordered table-sm bg-white text-dark">
                ...
                <tbody>
                  {loans
                    .filter((loan) => loan.loan_type !== "returns")
                    .map((loan) => (
                      <tr
                        key={loan.id}
                        className={
                          editingLoan?.id === loan.id ? "table-warning" : ""
                        }
                      >
                        ...
                        <td>
                          <span
                            className={`badge ${
                              loan.loan_type === "old"
                                ? "bg-primary"
                                : loan.loan_type === "today"
                                  ? "bg-success"
                                  : loan.loan_type === "ingoing"
                                    ? "bg-info"
                                    : loan.loan_type === "outgoing"
                                      ? "bg-warning"
                                      : loan.loan_type === "grn_damage"
                                        ? "bg-danger"
                                        : "bg-secondary"
                            }`}
                          >
                  ...
                  {loans.filter((loan) => loan.loan_type !== "returns")
                    .length === 0 && (
                    <tr>
          ...
          <div className="d-flex flex-wrap align-items-center gap-2 mb-3 mt-4">
            <button className="btn btn-dark">ණය වාර්තාව</button>
            <button className="btn btn-dark">නැවත ලබා දීම් වාර්තාව</button>
            <button className="btn btn-dark">චෙක් ගෙවීම් වාර්තාව බලන්න</button>
            <button className="btn btn-dark">Set Balance</button>
          </div>
        </div>
      </div>
    </div>
