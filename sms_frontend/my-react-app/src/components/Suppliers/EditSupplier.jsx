...
import api from "../../api"; // ✅ FIXED: Changed to api to pass Auth Tokens correctly
...
const EditSupplier = () => {
  ...
  const loadSupplier = async () => {
    try {
      // ✅ FIXED: Using direct 'api' call to prevent auto-logout
      const response = await api.get(`/suppliers/${id}`);
      const supplier =
        response.data.supplier || response.data.data || response.data;
      ...
      setPreviews({
        profile_pic: supplier.profile_pic
          ? STORAGE_URL + supplier.profile_pic
          : null,
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.nic_front instanceof File)
      data.append("nic_front", formData.nic_front);
    if (formData.nic_back instanceof File)
      data.append("nic_back", formData.nic_back);

    try {
      // ✅ FIXED: Using 'api.post' with FormData for correct update processing
      await api.post(`/suppliers/${id}`, data, {
  ...
  return (
    <div
      style={{ backgroundColor: "#99ff99", minHeight: "100vh", width: "100%" }}
    >
      <Sidebar />

      <div style={{ marginLeft: "260px", padding: "60px 40px" }}>
        {/* 🟢 STYLE BLOCK FOR LARGER FONTS 🟢 */}
        <style>{`
                    .form-label { font-size: 1.5rem; font-weight: bold; color: #ffffff; }
                    .form-control { font-size: 1.4rem; font-weight: bold; padding: 12px; }
                    .btn-lg { font-size: 1.4rem !important; font-weight: bold; padding: 12px 30px !important; }
                `}</style>

        <div className="col-12">
          <div
            className="p-5 rounded-4 shadow-lg text-light"
